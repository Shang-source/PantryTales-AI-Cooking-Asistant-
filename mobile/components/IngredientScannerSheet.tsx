...
interface IngredientScannerSheetProps {
  visible: boolean;
  onClose: () => void;
...
export function IngredientScannerSheet({
  visible,
  onClose,
  onTakePhoto,
  onSelectFromGallery,
}: IngredientScannerSheetProps) {
  ...
  const handleSelectFromGallery = async () => {
    ...
    if (!result.canceled && result.assets[0]) {
      onClose();
      // Wait for native animation to finish (camera/gallery dismissal)
