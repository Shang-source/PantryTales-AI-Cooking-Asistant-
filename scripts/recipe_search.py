...
st.title("Content Recipe Search & Recommendation")
...
tags_selected = st.multiselect(
    "Please select any tags for a meal you are interested in:", tag_options
)
...
ing_selected = st.text_input(
    "Please enter any ingredients you have on hand (separated by commas):",
    help="Use lowercase and plural form when appropriate",
)
...
if search:
    ...
    if tags_selected:
        recipes_rec["tag_match"] = recipes_rec["tags"].apply(
            all_tags_present, selected=tags_selected
        )
        recipes_rec = recipes_rec[recipes_rec["tag_match"]]
    if ing_selected:
        recipes_rec["ing_match"] = recipes_rec["ingredients"].apply(
            check_ingredients_df
        )
        recipes_rec = recipes_rec[recipes_rec["ing_match"]]
...
id_num = st.number_input(
    "Does one of these recipes catch your eye? Enter the id number here.", value=0
)
...
if sim:
    ...
    sim_scores = list(zip(recipes_feat["id"], cosine_sim[0]))
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)
    sim_scores = sim_scores[1:6]  # Get top 5 similar recipes
    rec_indices = [i[0] for i in sim_scores]

    recs = recipes_steps[recipes_steps["id"].isin(rec_indices)][
        ["id", "name", "description"]
    ]
    st.write(recs)
